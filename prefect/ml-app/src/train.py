...
### Define Process #############################################################
@flow(name="TrainBestModel")
def main():
    with timer("Load&Preprocess Data"):
        global X, y
        df_raw = load_data()
        df_proc = preprocess(df_raw)
        X, y = drop_unnecessary_column(df_proc)
        count_train_data_count(y)
...
@task
def load_data():
    return pd.read_csv(pj_dir / "data/01_raw/train.csv")
...
@task
def wrapper_optimizer():
    optimizer()


@hydra.main(config_path=f"{pj_dir}/src/config", config_name="config")
def optimizer(config: DictConfig) -> np.float64:
    cv = StratifiedKFold(n_splits=config.common.fold, shuffle=True, random_state=config.common.seed)
    ...
    with timer("CrossValidScore", logger):
        scores = cross_validate(model, X, y, scoring=list(config.metrics), cv=cv, fit_params=fit_params, n_jobs=-1)
...
def train_by_best_params() -> None:
    ...
    if optimization_results_file.exists():
        ...
        searched_params = {k.replace("model.train_params.", ""): v for k, v in searched_params.best_params.items()}
